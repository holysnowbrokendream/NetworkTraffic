...
1. 如果您还未阅读README.md文件，请先阅读以知晓项目整体部署流程，本指南不包含项目部署的所有操作。
2. 本指南在README.md的基础上，将帮助您将项目部署到内网环境，使同一局域网内的其他设备能够通过网络访问您的应用。
3. **推荐使用Docker镜像部署方式**，该方式自动支持内网访问，无需额外配置。

### 🌟 内网部署方式对比

| 部署方式 | 优势 | 适用场景 |
|----------|------|----------|
| **Docker镜像部署** | ✅ 自动检测IP，支持内网访问<br>✅ 无需重新构建，部署快速<br>✅ 配置简单，适合客户部署 | 🎯 **推荐用于内网部署** |
| 源码构建部署 | ✅ 可自定义配置<br>✅ 支持开发调试 | 🔧 开发环境或需要自定义配置 |
...
1. **导入预构建镜像**
   ...
   # 或
   ./import-images.sh  # Linux
   ```

2. **自动配置环境**
   ```bash
   cd ..
   images-setup.bat  # Windows
   # 或
   ./images-setup.sh  # Linux
   ```
   **脚本功能：**
   - 自动检测本机IP地址
   - 生成环境配置文件 (.env)
...
3. **启动服务**
   ```bash
   start-services-images.bat  # Windows
   # 或
   ./start-services-images.sh  # Linux
   ```
...
**无需额外配置！** Docker镜像部署方式会自动处理内网访问，包括：
- ✅ 自动检测主机IP
- ✅ 自动配置网络访问
- ✅ 自动支持内网设备访问
...
```bash
# 检查前端容器状态
docker-compose -f docker-compose.client.yml ps frontend
...
```bash
# 检查后端容器状态
docker-compose -f docker-compose.client.yml ps backend
...
```bash
# 检查MySQL容器状态
docker-compose -f docker-compose.client.yml ps mysql

# 查看MySQL日志
docker-compose -f docker-compose.client.yml logs mysql

# 重启MySQL服务
docker-compose -f docker-compose.client.yml restart mysql
```
